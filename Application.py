...
def send_email():
    ...
    root.filename = \
        ...
        (("all files","*.*"),("jpeg files","*.jpg"),("png files","*.png"),("pdf files","*.pdf"),
         ("text files","*.txt"),("xlsx files","*.xlsx"),("docx files", "*.docx")))
...
button_show = tk.Button(root, text="Show Email", command=show_data)
button_show.pack()

# Create a tkinter button to trigger the function to send email
button_email = tk.Button(root, text="Send Email", command=show_data)
button_email.pack()
