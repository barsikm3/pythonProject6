...
def search(*args):
    ...
    for i, e in matching:
        results_list.insert(tk.END, f"Row {i+1}: " + ", ".join([df.columns[j], str(e)]))
...
def select_email(*args):
    ...
    if selection:
        ...
        select_data.set(f"Row {selected_row + 1}: " + ", ".join([f"{col}: {val}" for col, val in zip(df.columns, selected_data)]))
